    ...
    if request.method == "POST":
        ...
        if action == "Upload":
            ...
            uploaded_images = alignforcheck(
                face_num_1, check_uploaded_images[0], uploaded_images
            )
            uploaded_images = alignforcheck(
                face_num_2, check_uploaded_images[1], uploaded_images
            )
            if face_num_1 == -2:
                path1 = os.path.join(app.config["UPLOAD_FOLDER"], image1)
            else:
                path1 = os.path.join(
                    app.config["ROOT_FOLDER"],
                    "static",
                    f"aligned_{face_num_1}_{image1}",
                )
            if face_num_2 == -2:
                path2 = os.path.join(app.config["UPLOAD_FOLDER"], image2)
            else:
                path2 = os.path.join(
                    app.config["ROOT_FOLDER"],
                    "static",
                    f"aligned_{face_num_2}_{image2}",
                )
